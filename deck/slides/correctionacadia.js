// Correction (ACADIA) — the shared correction scene, opened on the real tool.
//
// Identical to the shared `correction` skeleton but for one extra beat at the
// front: the Rollout Inspector, as a still of the running web app, flying in on
// arrival and back out as the replay begins. Nothing animates inside it — the
// abstract replay that follows is what does the explaining. The point is only
// that the audience has seen the actual tool before being shown a diagram of
// what it does, which is the same move the ACADIA demonstration slide makes.
//
// The shell comes from tools/export_acadia_collection_assets.py, run with
// --no-blank because nothing is drawn over this one.

export const meta = { title: "Correction (ACADIA)", defaultMinutes: 2.5 };
export const sectionClass = "section--correction";
export const sectionId = "teaching-correction-acadia";
const SHELL = "assets-static/correction-acadia";
...
// Each of these three presses draws its bars in, rather than switching state,
// so the build reads as a build in a static format too.
...
export const beats = [
  // Sits at 0 so it is the arrival state rather than an extra stop: the scene's
  // own phase boundaries below are untouched, and the shell is simply gone by
  // the time the replay beat is reached.
  { name: "inspector", p: 0.0 },
  ...
  return `
    <div class="correction-stage">
      <div class="container correction-stage__inner">
        ...
        <div class="os-window os-window--wide wp-still" id="correction-shell">
          <div class="os-window__bar">
            ...
            <span class="os-window__label">Rollout Inspector</span>
          </div>
          <div class="os-window__body os-window__body--dark">
            <img src="${SHELL}/shell.png" alt="The Rollout Inspector web app, replaying a failed rollout"
              decoding="async">
          </div>
        </div>
...
export async function mount(root, props) {
  ...
  const shellLayer = section.querySelector("#correction-shell");
  const headEl = section.querySelector(".correction-head");
  const innerEl = section.querySelector(".correction-stage__inner");

  /**
   * Centre the shell in the space the copy leaves, not on the slide.
   *
   * The copy card is pinned to the left of this stage, so the middle of the
   * slide is not the middle of the space the shell has to sit in — at 1920 the
   * two are 224px apart. Measured from the elements themselves rather than
   * derived from the copy's max-width, which is a rem pin and would drift the
   * moment either the card or the frame changed.
   */
  function placeShell() {
    if (!shellLayer || !headEl || !innerEl) return;
    const inner = innerEl.getBoundingClientRect();
    const head = headEl.getBoundingClientRect();
    if (!inner.width) return;
    const free = inner.right - head.right;      // what is left once the copy has its column
    const centre = head.right - inner.left + free / 2;
    shellLayer.style.left = `${centre.toFixed(1)}px`;
  }
  ...
  function renderLayers(p) {
    // The shell owns the stage until the replay beat at 0.18, and is fully gone
    // by 0.16 so the two never cross-fade into a muddle. The replay layer is
    // held back behind it rather than starting underneath.
    const shellOut = smooth(p, 0.02, 0.16);
    shellLayer.style.opacity = String(1 - shellOut);
    shellLayer.style.transform = `scale(${1 - 0.06 * shellOut})`;
    shellLayer.style.pointerEvents = shellOut > 0.5 ? "none" : "auto";

    const out = smooth(p, 0.58, 0.72);
    replayLayer.style.opacity = String(Math.min(shellOut, 1 - out));
  ...
  if (!full || window.innerWidth < 760) {
    ...
    if (shellLayer) shellLayer.style.opacity = "0";
