// Correction (IASS) — the same two-layer scene as the shared `correction`
// skeleton, on the IASS experiment's data.
//
// The shared skeleton shows the ACADIA study: four correction rounds, grouped by
// chasm width (8 / 10 / 12 m), replaying an 8 m takeover. IASS is a different
// experiment — a single 10 m span, six rounds, and demonstrations spanning five
// bar budgets. So the grid here is organised by bar count (11 / 13 / 15 / 17 /
// 19), two diverse examples each, and rows fade lighter with the round index.
//
// Data: assets-static/correction-iass/, exported from the IASS correction store
// by tools/export_iass_correction_assets.py. Seven of the sixty slots are empty
// on purpose: round 2 collected no 19-bar corrections, round 3 no 11-bar ones,
// and rounds 3-5 only one 19-bar each. The gaps are the data, not a bug.

export const meta = { title: "Correction (IASS)", defaultMinutes: 2.5 };
export const sectionClass = "section--correction";
export const sectionId = "teaching-correction-iass";
...
export const defaults = {
  ...
  datasetFoot: "six correction rounds \u00b7 10 m span \u00b7 retraining",
  eyebrow: "Correction",
      ...
      <div class="container correction-stage__inner">
        ...
        <div class="correction-dataset" id="correction-dataset" aria-hidden="true">
          <div class="dataset-header">
            ...
            <!-- One chip per bar budget, built from the manifest so the legend
                 cannot drift from the tints the cells actually use. -->
            <ul class="dataset-legend" id="correction-legend"></ul>
          </div>
...
const BASE = "assets-static/correction-iass";
...
export async function mount(root, props) {
  ...
  const countBg = manifest.barcount_backgrounds || {};

  // Fade the bar-count tint by the round alpha, composited over white so it
  // reads the same regardless of the orange section background (matches the
  // plot). Round 1 is full-strength; each later round lands lighter.
  ...
  // Legend: one chip per bar budget, at full tint strength.
  const legendEl = section.querySelector("#correction-legend");
  if (legendEl) {
    legendEl.innerHTML = "";
    for (const n of manifest.bar_counts || []) {
      const li = document.createElement("li");
      li.innerHTML = `<span class="dataset-legend__chip" style="--c: ${
        countBg[String(n)] || "#fff"
      };"></span>${n} bars`;
    ...
    rowsData.forEach((row, i) => {
      ...
      const addCells = () => {
        row.cells.forEach((c) => {
          ...
          const bg = countBg[String(c.bars)];
          if (bg) cell.style.setProperty("--tint", compositeOverWhite(bg, row.alpha ?? 1));
          if (c.placeholder) {
            // No such demo was collected in this round at this bar budget.
            // Hold the slot so the columns stay aligned across rows.
            cell.classList.add("dataset-cell--empty");
          } else {
      ...
      addCells(); // measureRows() appends however many more copies the wrap needs
  ...
  function measureRows() {
    for (const rt of rowTracks) {
      const kids = rt.track.children;
      if (!kids.length || !rt.count) continue;
      const gap = parseFloat(getComputedStyle(rt.track).gap) || 0;
      const cellW = kids[0].getBoundingClientRect().width;

      // How far the track must travel for copy N+1 to sit exactly where copy N
      // started. Note the trailing gap: scrollWidth / 2 counts one gap fewer
      // than there are cells, so wrapping on it drops the last thumbnail half a
      // gap short of the first and the loop visibly ticks once per cycle.
      const period = rt.count * (cellW + gap);

      // A row must stay covered right up to the wrap, so the track needs to be
      // at least one viewport plus one period wide — two copies leaves a blank
      // tail creeping in from the right for the back half of every cycle.
      const copies = Math.max(2, Math.ceil((rt.viewport.clientWidth + gap) / period) + 1);
      while (kids.length / rt.count < copies) rt.addCells();
